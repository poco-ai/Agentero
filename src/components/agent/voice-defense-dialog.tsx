...
const RUNNING_PREPARATION_STATUSES = new Set([
	"created",
	"snapshotting",
	"analyzing",
	"synthesizing",
...
function readStoredScenario(): VoiceScenario {
...
function writeStoredScenario(scenario: VoiceScenario): void {
...
function readStoredLanguage(fallback: "en" | "zh-CN"): "en" | "zh-CN" {
...
function writeStoredLanguage(language: "en" | "zh-CN"): void {
	if (typeof localStorage === "undefined") return;
	try {
...
function readStoredDifficulty(): VoiceDifficulty {
...
function writeStoredDifficulty(difficulty: VoiceDifficulty): void {
	if (typeof localStorage === "undefined") return;
	try {
...
const MIN_CUSTOM_DURATION_MINUTES = 1;
const MAX_CUSTOM_DURATION_MINUTES = 240;

function clampPlannedDurationMinutes(value: number): number | null {
...
function readPlannedDurationMinutes(): number | null {
	if (typeof localStorage === "undefined") return null;
	try {
		const raw = localStorage.getItem(PLANNED_DURATION_STORAGE_KEY);
		if (!raw) return null;
...
function writePlannedDurationMinutes(minutes: number | null): void {
	if (typeof localStorage === "undefined") return;
	try {
		if (minutes === null) {
			localStorage.removeItem(PLANNED_DURATION_STORAGE_KEY);
			return;
		}
		...
		localStorage.setItem(PLANNED_DURATION_STORAGE_KEY, String(clamped));
	} catch {
		// Best-effort preference; the session still runs without it.
	}
}

type PreparationRecoveryRecord = {
	vaultPath: string;
	paperPath: string;
	runId: string;
};

function readPreparationRecovery(): PreparationRecoveryRecord | null {
	if (typeof sessionStorage === "undefined") return null;
	try {
		const raw = sessionStorage.getItem(PREPARATION_RECOVERY_STORAGE_KEY);
		if (!raw) return null;
		...
		if (
			...
			sessionStorage.removeItem(PREPARATION_RECOVERY_STORAGE_KEY);
			return null;
		}
		return {
...
function writePreparationRecovery(record: PreparationRecoveryRecord): void {
	if (typeof sessionStorage === "undefined") return;
	try {
		sessionStorage.setItem(
			PREPARATION_RECOVERY_STORAGE_KEY,
...
function clearPreparationRecovery(runId?: string): void {
	if (typeof sessionStorage === "undefined") return;
	try {
		if (!runId || readPreparationRecovery()?.runId === runId) {
			sessionStorage.removeItem(PREPARATION_RECOVERY_STORAGE_KEY);
		}
	} catch {
...
function preparationFailureDescription(error: unknown): string {
	if (!(error instanceof DefensePreparationFailedError)) {
		return error instanceof Error ? error.message : String(error);
	}
	// Only the latest attempt of each node explains this failure; older
	// attempts (earlier cancellations, retried timeouts) are just noise.
...
export function VoiceDefenseDialog({
	windowMode = false,
	vaultPath,
	currentFileLabel,
	focusedMaterialPath,
	...
	const voiceLeaseRef = useRef<string | null>(null);
	const voiceStartOperationRef = useRef(0);
	...
	const releaseOwnedVoiceSession = useCallback((leaseId: string | null) => {
		...
		if (voiceLeaseRef.current === leaseId) voiceLeaseRef.current = null;
	}, []);
	...
	useEffect(() => {
		return () => {
			...
			voiceStartOperationRef.current += 1;
	...
	const reset = useCallback(() => {
		...
		if (voiceLeaseRef.current && !clientRef.current) {
			releaseOwnedVoiceSession(voiceLeaseRef.current);
		}
		setPhase("prepare");
	...
	const openDialog = () => {
		...
		const canRecover = Boolean(
			recovery &&
				...
				focusedMaterialPath === recovery.paperPath,
		);
		if (canRecover && recovery) {
			...
			void loadDefensePreparation(recovery.vaultPath, recovery.runId)
				.then(async (manifest) => {
					...
					setPreparationError(
						refreshed.status === "failed" || refreshed.status === "cancelled",
					);
					setSelectedMaterialPaths(
	...
	const beginVoiceStart = (): number | null => {
		...
		if (!acquireVoiceDefenseSession(leaseId)) return null;
		voiceLeaseRef.current = leaseId;
		voiceStartingRef.current = true;
		setVoiceStarting(true);
		voiceStartOperationRef.current += 1;
		return voiceStartOperationRef.current;
	};
	...
	const releaseVoiceStart = (operation: number) => {
		if (voiceStartOperationRef.current !== operation) return;
		voiceStartingRef.current = false;
		setVoiceStarting(false);
		if (voiceLeaseRef.current && !clientRef.current) {
			releaseOwnedVoiceSession(voiceLeaseRef.current);
	...
	const runPreparation = async (resume: boolean) => {
		if (voiceStartingRef.current || isVoiceDefenseSessionActive()) return;
	...
	const startVoice = async (
		...
		if (!vaultPath || !material.trim() || !authStatus?.connected || !isTauri())
			return false;
		if (
			hasActiveDefensePreparations() ||
			hasActivePreparationChildren() ||
		...
		if (!isCurrentVoiceStart(operation) || voiceLeaseRef.current !== leaseId) {
			return false;
		}
		setContext(material);
		...
		startedAtRef.current = null;
		setStartedAt(null);
		const markVoiceStarted = (activity: "speaking" | "caption") => {
		...
		const client = new VoiceDefenseClient({
			onStatus: (status) => {
				if (clientRef.current !== client || voiceLeaseRef.current !== leaseId) {
					return;
				}
				if (status === "speaking") markVoiceStarted("speaking");
	...
	const startWithPreparedMaterial = async () => {
		if (!vaultPath || !preparation || !context.trim()) return;
		...
		try {
			...
			if (voiceStartOperationRef.current === operation && openRef.current) {
	...
	const endSession = useCallback(async () => {
		...
		const finishOperation = voiceStartOperationRef.current;
		finishingRef.current = finishOperation;
		const isCurrentFinish = () =>
			finishingRef.current === finishOperation &&
			voiceStartOperationRef.current === finishOperation;
		const finishDebrief = () => {
			const composed =
	...
	const handleOpenChange = (next: boolean) => {
		...
		// A live call (or a failed one with captions) never closes silently:
		// end the session and land on the debrief so the user decides whether
		// the transcript is worth saving. Nothing is written automatically.
		if (
			phase === "live" ||
	...
	const shell = (
		<div
			...
			role="dialog"
			aria-modal={windowMode ? undefined : true}
			aria-label={t("voiceDefense.title")}
		>
			{phase === "prepare" ? (
				<SetupAct
					title={title}
					...
					onCancelAccountConnection={() => void cancelAccountConnection()}
					onDisconnectAccount={() => void disconnectAccount()}
					materials={selectedMaterials}
					materialOptions={filteredMaterialOptions}
					currentMaterialPath={focusedMaterialPath}
					selectedMaterialPaths={selectedMaterialPaths}
					materialSearch={materialSearch}
					onMaterialSearchChange={setMaterialSearch}
					onToggleMaterial={toggleMaterial}
					inputsLocked={preparationInputsLocked}
					instruction={instruction}
					onInstructionChange={setInstruction}
					plannedMinutes={plannedMinutes}
					onPlannedMinutesChange={selectPlannedMinutes}
					scenario={scenario}
					onScenarioChange={selectScenario}
					defenseLanguage={defenseLanguage}
					onDefenseLanguageChange={selectDefenseLanguage}
					difficulty={difficulty}
					onDifficultyChange={selectDifficulty}
					reusable={
						reusableManifest
							? {
									updatedAt: reusableManifest.updatedAt,
									partial: reusableManifest.partial,
								}
							: null
					}
					onUseReusable={useReusablePreparation}
					history={history}
					onOpenHistory={(path) => onOpenSource?.(path)}
					...
					preparationStale={Boolean(preparation?.stale)}
					selectionChanged={selectionChanged}
					voiceStarting={voiceStarting}
					brief={context}
					onBriefChange={setContext}
					briefSource={source}
					onPrepare={() => void runPreparation(false)}
					onRetry={() => void runPreparation(Boolean(preparation))}
					onCancelPreparation={() => void cancelPreparation()}
					onStart={() => void startWithPreparedMaterial()}
				...
				<EndedAct
					...
					canRestart={Boolean(context.trim()) && !voiceStarting}
					onSaveTranscript={
						vaultPath && captions.length > 0 && !savedPath
							? () => void saveTranscript()
							: null
					}
					onOpenTranscript={
						savedPath && onOpenSource ? () => onOpenSource(savedPath) : null
					}
					onEvaluate={
						vaultPath && captions.length > 0 && !reviewPath
							? () => void generateReview()
							: null
					}
					onOpenReview={
						reviewPath && onOpenSource ? () => onOpenSource(reviewPath) : null
					}
					onRestart={() => void restartDefense()}
					onClose={() => handleOpenChange(false)}
				/>
			) : (
				<LiveAct
					...
					onSendPatch={sendLivePatch}
					onRefocus={sendLiveRefocus}
					onEnd={() => void endSession()}
					onCancelConnecting={() => handleOpenChange(false)}
					onRetry={() => setPhase("prepare")}
					onClose={() => handleOpenChange(false)}
				/>
			)}
			<audio ref={audioRef} autoPlay muted className="sr-only">
				<track kind="captions" src="data:text/vtt,WEBVTT%0A" />
			</audio>
		</div>
	);
	return windowMode ? shell : createPortal(shell, document.body);
}
