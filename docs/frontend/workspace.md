...
| `src/lib/workspace/selection-overlay.ts` | 划词工具栏 / Ask 卡等浮动浮层的挂载计数；工作区据此在浮层打开时 `disableDnd`（#608） |
