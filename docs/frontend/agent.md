...
- **单账号内置登录**：答辩间提供“连接 ChatGPT”。点击后由 Host 创建临时原生 `voice-auth` WebView；前端只监听连接状态，不接收 ChatGPT token。成功捕获会话并写入系统凭证库（macOS Keychain、Windows Credential Manager、Linux Secret Service）后登录窗口会被彻底销毁。macOS 上若条目由另一构建（dev / 生产包）写入，断开或覆盖会先走系统 `security` 工具，避免 Keychain ACL 拒绝。「开始答辩」只以账号连接、Vault 和上下文状态为前置条件；上游返回 401 时 Host 会删除失效凭证并让答辩间回到重新连接状态。设置页不暴露 Gateway URL、API Key 或连接测试。
...
- 实时答辩与会前准备：`src/components/agent/voice-defense-dialog.tsx`（三幕 UI）、`src/components/agent/use-voice-defense.ts`（编排）、`src/lib/voice-defense/`、`src/lib/voice-defense/preparation/`
