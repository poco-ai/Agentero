...
- 魔棒输入的类型识别完全在 Host 侧：`crates/agentero-core/src/features/paper/import/skill/source.rs` 的 `extract_skill_source()`（`SkillSource`，识别为 `skill` kind，不入 resolver 表）和 `parse::extract_primary_identifier()`（其余标识符走 `resolver.rs` 静态表）。Skill 识别发生在普通 URL 分支之前，GitHub 链接不会再被送入 Translator `/web`。
...
`parses_requested_skill_import_examples`，`parse.rs` 仍保留
