...
| Agent | 订阅详情与 arXiv Daily 远程 PDF 均支持划词提问 / 加入对话（#421；远程 ephemeral，不写 `marks/`）；Agent `@` 可引用广场条目（arXiv Daily 当日 + Feeds 最近条目，元数据展开进 prompt，见 [agent.md](../frontend/agent.md)） |
...
| **P2 @ 广场条目（已交付）** | Agent `@` 搜索引用 arXiv Daily + Feeds 条目；虚拟路径 `agentero:plaza/…`，发送时展开元数据 | 标题关键词可命中；条目过期降级提示，不静默丢弃 |
